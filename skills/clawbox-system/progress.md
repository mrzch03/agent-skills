...
## 进行中
- **Android 复刻 + SDK 化立项**(2026-07-04 创始人新任务):SDK 优先架构——核心进 `:tapio-sdk` AAR,`:app` 薄壳 demo;纯 Kotlin+Compose。存量 6000 行(旧 iOS 对应版)构建基线绿✓;作战计划(盘存/差距/P1会话心脏→P2产品面→P3打包/各阶段验收)在 Tapio 仓 `docs/android-sdk-plan.md`。下一步 P1:模块拆分 + live 会话追平(kinds/PTT前后滚/语音条/黑板卡/重连)。
